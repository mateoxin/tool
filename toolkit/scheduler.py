...
def get_lr_scheduler(
    ...
    if name == "cosine":
        ...
        # Remove control flags that are not PyTorch scheduler parameters
        if 'max_iterations' in kwargs:
            kwargs.pop('max_iterations')
        
        if 'total_iters' in kwargs and 'T_0' not in kwargs:
            # Only use total_iters if T_0 is not explicitly set
            kwargs['T_0'] = kwargs.pop('total_iters')
        elif 'total_iters' in kwargs:
            # T_0 is explicitly set, just remove total_iters
